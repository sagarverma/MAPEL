...
def select_action(model, state):
    ...
    if sample > eps_threshold:
        return model(Variable(state, volatile=True).cuda(0).type(FloatTensor)).data.max(1)[1].view(1, 1)
    else:
        return LongTensor([[random.randint(0,7)]])
...
def run_episode(e, environment):
    ...
    while True:
        ...
        guard_action_locs, _ = environment.act()
...
w = csv.writer(open('logs/invader_dqn_guards_simple.csv','wb'))
...
for e in range(EPISODES):
    ...
    guard1 = Guard(id=70, speed=1, obs_size=5)
    guard2 = Guard(id=71, speed=1, obs_size=5)
    target = Target(speed=0)

    env = NEnvironment([32,32], [guard1, guard2], [invader1, invader2], target, sim_speed=0.00001)

    obs = env.render()
    cv2.imwrite('teaser.png', cv2.resize(obs, (500,500), interpolation=cv2.INTER_NEAREST))
    exit()
    run_episode(e, env)
...
torch.save(invader_models[80], 'weights/invader1_against_simple_guards.pt')
torch.save(invader_models[81], 'weights/invader2_against_simple_guards.pt')

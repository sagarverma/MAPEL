...
invader_model_conv = SimpleNet(num_channels, history_length, num_actions)
invader_model_conv = torch.load(weights_dir + dataset_name + '_invader_sim.pt')

guard_model_conv = SimpleNet(num_channels, history_length, num_actions)
guard_model_conv = torch.load(weights_dir + dataset_name + '_guard_sim.pt')

if use_gpu:
    invader_model_conv = invader_model_conv.cuda(DEVICE)
    guard_model_conv = guard_model_conv.cuda(DEVICE)
    
invader_wins = 0
guard_wins = 0
for i in range(100):

    invader = NInvader(invader_model_conv, speed=1)
    guard = NGuard(guard_model_conv, speed=1)
...
print ('invader wins:', invader_wins, 'guard wins:', guard_wins)
...
class CNNet(nn.Module):
    def __init__(self, in_channels=5, num_actions=8):
...
def select_invader_action(model, state):
    global invader_steps_done
    sample = random.random()
    eps_threshold = EPS_END + (EPS_START - EPS_END) * math.exp(-1. * invader_steps_done / EPS_DECAY)
    if sample > eps_threshold:
        return model(Variable(state, volatile=True).type(FloatTensor)).data.max(1)[1].view(1, 1)
    else:
        return LongTensor([[random.randrange(8)]])

def select_guard_action(model, state):
    global guard_steps_done
    sample = random.random()
    eps_threshold = EPS_END + (EPS_START - EPS_END) * math.exp(-1. * guard_steps_done / EPS_DECAY)
    if sample > eps_threshold:
        return model(Variable(state, volatile=True).type(FloatTensor)).data.max(1)[1].view(1, 1)
    else:
...
def run_episode(e, environment):
    ...
    while True:
        ...
        for invader in invader_models.keys():
            invader_actions[invader] = select_invader_action(invader_models[invader], FloatTensor([invader_states[invader]]))
        invader_steps_done += 1
        guard_actions = {}

        for guard in guard_models.keys():
            guard_actions[guard] = select_guard_action(guard_models[guard], FloatTensor([guard_states[guard]]))
        guard_steps_done += 1
...
invader_models = {80: CNNet(num_actions=8), 81: CNNet(num_actions=8)}
guard_models = {70: CNNet(num_actions=9), 71: CNNet(num_actions=9)}
...
for e in range(EPISODES):
    ...
    env = Environment([32,32], [guard1, guard2], [invader1, invader2], target, sim_speed=0.00001)
    run_episode(e, env)
